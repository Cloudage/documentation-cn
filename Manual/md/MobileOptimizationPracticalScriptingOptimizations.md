...
````
如果同时运行的数量足够多，可能会很慢。鲜为人知的一个事实：MonoBehaviour 中的所有组件访问者（比如**变换**、**渲染器**和**音频**）都等同于它们的 **GetComponent(Transform)** 对应项，它们实际上有点慢。**GameObject.FindWithTag** 已经过优化，但在某些情况下（例如，在内部循环中或在许多实例上运行的脚本上），此脚本可能有点慢。
